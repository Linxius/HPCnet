...
class HPC_SAModuleMSG(nn.Module):
    def __init__(self, npoint, radius_list, nsample_list, in_channel, mlp_list):
        ...
        if WITH_W:
            self.conv_blocks_ = nn.ModuleList()
            self.bn_blocks_ = nn.ModuleList()
        for i in range(len(mlp_list)):
            convs = nn.ModuleList()
            bns = nn.ModuleList()
            last_channel = in_channel + 3 + HPC_LEN
            ...
            if WITH_W:
                ...
                for out_channel in mlp_list[i]:
                    ...
                    last_channel = out_channel
                self.conv_blocks_.append(convs_)
                self.bn_blocks_.append(bns_)
    ...
    def forward(self, xyz, points):
        ...
        for i, radius in enumerate(self.radius_list):
            ...
            if WITH_W:
                for j in range(len(self.conv_blocks[i])):
                    conv = self.conv_blocks_[i][j]
                    bn = self.bn_blocks_[i][j]
                    points_w =  F.relu(bn(conv(points_w)))
                grouped_points = (1+points_w)*grouped_points
